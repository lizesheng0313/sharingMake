    ...
    handleSearch() {
      ...
      for (let i = 0; i < this.activeList.length; i++) {
        for (let j = 0; j < this.screeningList[i].list.length; j++) {
          if (this.activeList[i][j] || this.activeList[i][j] == 0) {
            if (i == 0) {
              let cur = screening.idValidStatus[j];
              this.ruleForm.idValidStatus.push(cur);
            } else if (i == 1) {
              let cur = screening.reportStatus[j];
              this.ruleForm.reportStatus.push(cur);
            } else if (i == 2) {
              console.log(screening.empStatus);
              console.log(j);
              let cur = screening.empStatus[j];
              console.log(cur);
              this.ruleForm.empStatus = cur;
            } else if (i == 3) {
              let cur = screening.iscgl[j];
              this.ruleForm.empStatus = cur;
            } else if (i == 4) {
              let cur = screening.workerType[j];
