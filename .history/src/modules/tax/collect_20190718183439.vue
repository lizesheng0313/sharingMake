      ...
      <el-dialog :visible.sync="isShowScreening" width="52%" center class="screen-dialog">
        <div class="screening-wapper">
          <el-row type="flex" class="screening-box" align="middle">
            ...
            <el-col>
              <span
                v-for="(value,key,index) in screening.idValidStatus"
                :key="index"
                @click="handleSelectScreening(key)"
              >{{value}}</span>
            </el-col>
          </el-row>
          <el-row type="flex" class="screening-box" align="middle">
            ...
            <el-col>
              <span
                v-for="(value,key,index) in screening.reportStatus"
                :key="index"
                @click="handleSelectScreening(key)"
              >{{value}}</span>
            </el-col>
  ...
  methods: {
    handleSearch() {
      ...
      for (let i = 0; i < this.activeList.length; i++) {
        for (let j = 0; j < this.screeningList[i].list.length; j++) {
          let current = this.activeList[i][j];
          if (current) {
            if (i == 0) {
              let cur = screening.idValidStatus[current];
              this.ruleForm.idValidStatus.push(cur);
            } else if (i == 1) {
              let cur = screening.reportStatus[current];
              this.ruleForm.reportStatus.push(cur);
            } else if (i == 2) {
              let cur = screening.empStatus[current];
              this.ruleForm.empStatus = cur;
            } else if (i == 3) {
              let cur = screening.iscgl[current];
              this.ruleForm.empStatus = cur;
            } else if (i == 4) {
              let cur = screening.workerType[current];
              this.ruleForm.workerType.push(cur);
            }
          }
        }
      }
    ...
    handleSelectScreening(key) {
      //根据不同的下标 来判断是多选还是单选
      if (index == 0 || index == 1 || index == 4) {
       this.
        // let cur = this.activeList[index].indexOf(idx);
        // if (cur == -1) {
        //   this.$set(this.activeList[index], idx, idx);
        // } else {
