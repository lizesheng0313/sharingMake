  ...
  <full-screen :fsTitle="'人员信息'" :bgColor="'#fff'" class="info-collection">
    <div slot="fs-container">
      <div class="state-personnel">
        <i>人员状态</i>
        <el-button plain :class="{active:currentState ==0}" @click="currentState = 0">正常</el-button>
        <el-button plain :class="{active:currentState ==1}" @click="currentState = 1">非正常</el-button>
      </div>
      <el-form
      ...
      >
        <div>
          ...
          <el-row type="flex" justify="center">
            <el-col :span="7">
              <el-form-item label="证件类型" prop="cert">
                <el-select v-model="contentList.idType" placeholder="请选择">
                  <el-option
                    v-for="(value,key,index) in baseInfo.documentType"
                    :key="index"
                    <!-- :label="key" -->
                    :value="value"
                  ></el-option>
...
import * as SCR from "./store/actionTypes";
export default {
  components: {
    fullScreen
  },
